...
FOR /L %%s IN (110,20,190) DO (
  ECHO %%s
  od2trips --configuration-file .\od2trips\od2trips.config.xml --taz-files .\TAZ\TAZ.taz.xml --od-matrix-files .\Matrice\OD_Montreal_5000.od.txt --seed %%s --output-prefix seed%%s --output-file ./Scenario/5000/od2trips/od_5000.odtrips.xml
  ...
  sumo.exe --seed %%s -c Simulation.sumocfg --net-file .\Carte\net\MontrealJointure.net.xml --route-files ./Scenario/5000/duarouter/seed%%sod_5000.odtrips.rou.xml  --output-prefix seed%%s- --summary ./Scenario/5000/summary/running
